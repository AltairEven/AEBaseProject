...
NSString *const kThirdPartyLoginTypeWechat = @"kThirdPartyLoginTypeWechat";
NSString *const kThirdPartyLoginTypeAlipay = @"kThirdPartyLoginTypeAlipay";
NSString *const kThirdPartyLoginTypeWeibo = @"kThirdPartyLoginTypeWeibo";
NSString *const kThirdPartyLoginTypeQQ = @"kThirdPartyLoginTypeQQ";
...
+ (NSArray<NSNumber *> *)availableLoginTypes {
    ...
    //微信
    if ([WeChatManager canLogin]) {
        [availableArray addObject:[NSNumber numberWithInteger:ThirdPartyLoginTypeWechat]];
    }
    //支付宝
    if ([AlipayManager canLogin]) {
        [availableArray addObject:[NSNumber numberWithInteger:ThirdPartyLoginTypeAlipay]];
    }
    //微博
    if ([WeiboManager canLogin]) {
        [availableArray addObject:[NSNumber numberWithInteger:ThirdPartyLoginTypeWeibo]];
    }
    //QQ
    if ([TencentManager canLogin]) {
        [availableArray addObject:[NSNumber numberWithInteger:ThirdPartyLoginTypeQQ]];
    }
...
+ (NSDictionary *)loginTypeAvailablities {
    ...
    //微信
    BOOL canLogin = [WeChatManager canLogin];
    [availableDic setObject:[NSNumber numberWithBool:canLogin] forKey:kThirdPartyLoginTypeWechat];
    //支付宝
    canLogin = [AlipayManager canLogin];
    [availableDic setObject:[NSNumber numberWithBool:canLogin] forKey:kThirdPartyLoginTypeAlipay];
    //微博
    canLogin = [AlipayManager canLogin];
    [availableDic setObject:[NSNumber numberWithBool:canLogin] forKey:kThirdPartyLoginTypeWeibo];
    //QQ
    canLogin = [TencentManager canLogin];
    [availableDic setObject:[NSNumber numberWithBool:canLogin] forKey:kThirdPartyLoginTypeQQ];

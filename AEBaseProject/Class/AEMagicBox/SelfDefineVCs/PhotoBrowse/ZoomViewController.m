 ...
 * 存放高清图的hignImage
 */
@property (nonatomic, strong) UIImage *hignImage;
...
- (instancetype)initWithImageView:(PhotoItem *)photoItem finish:(finishBlock)block;
{
    if (self = [super init])
    {
        ...
        self.hignImage = photoItem.hignImage;
        self.block = block;
        self.isLoaded = YES;
        isZoom = NO;
...
- (void)viewAppearAnimate:(NSString *)value
{
    if ([value isEqualToString:@"appear"])
    {
        ...
        if (self.url) {
            __weak UIImageView *weakBanner = self.imageView;
            [weakBanner sd_setImageWithURL:self.url placeholderImage:[UIImage imageNamed:@""] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if (!error) {
                    weakBanner.image = image;
                    weakBanner.frame = [self getFrameByImageSize:image.size];
                }
            }];
            self.imageView = weakBanner;
            self.isLoaded = YES;
        }
        else
        {
        self.imageView.image = self.hignImage;
        self.isLoaded = YES;
        self.imageView.frame = [self getFrameByImageSize:self.hignImage.size];
        self.zoomView.contentSize = CGSizeMake(self.view.frame.size.width, self.view.frame.size.width / scale(self.hignImage.size));
        }
